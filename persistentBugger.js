...
function persistence(num) {
    ...
    var numArray = numStringArray.map(x=>Number(x));
    if(num<10) {
        return 0;
    }
    if(num>10) {
        var counter = 0;
        for(var i=0; numArray.length>1; i++) {
            var prod = numArray.reduce((a,b)=>a*b,1);
            numString = prod.toString();
            numStringArray = numString.split("");
            numArray = numStringArray.map(x=>Number(x));
            counter = counter +1;
        }
        return counter;
