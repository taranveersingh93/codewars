...
As the name may already reveal, it works basically like a Fibonacci, but summingt
 he last 3 (instead of 2) numbers of the sequence to generate the next. And, worse 
 part of it, regrettably I won't get to hear non-native Italian speakers trying to pronounce it :(
...
function tribonacci(signature,n){
    ...
    if(n<=3) {
        for(var i=0; i<n; i++) {
            outputArray.push(signature[i]);
        }
    } else {
        outputArray = [...signature];
        for(var i=3; i<n; i++) {
            outputArray.push(outputArray[i-1]+outputArray[i-2]+outputArray[i-3]);
        }
    }
    return outputArray;
}